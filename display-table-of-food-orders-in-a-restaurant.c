...
char***displayTable(char***orders, int ordersSz, int*orders0sz, int*rsz, int**csz){
    int d = 0;
    int t = 0;
    char*ddish [ordersSz]; int dsz=0;
    int  ttable[ordersSz]; int tsz=0;
    msi*dish  = newMap();
    msi*table = newMap();
    for(int z=0; z<ordersSz; z++){char**vec = orders[z];
        ddish [dsz++] = vec[2];
        ttable[tsz++] = atoi(vec[1]);
    }
    qsort(ddish,  dsz, sizeof(char*), cmpstr);
    qsort(ttable, tsz, sizeof(int)  , cmp);
    dsz = unique_s(ddish , dsz);
    tsz = unique  (ttable, tsz);
    for(int z=0; z<dsz; z++){char*i = ddish[z];
        mapPut(dish, i, ++d);
    }
    for(int z=0;z<tsz;z++){int i = ttable[z];
        mapPut(table, to_string(i), ++t);
    }
    char***res = calloc(t+1, sizeof(char**));
    for(int z=0; z<t+1; z++)
        res[z] = calloc(d+1, sizeof(char*));
    int cnt[t+1][d+1]; clr(cnt);
    res[0][0] = strdup("Table");

    void doit(kvp*p){
        res[0][p->val] = p->key;
    }
    mapForEach(dish, doit);

    void d0it(kvp*p){
        res[p->val][0] = p->key;
    }
    mapForEach(table, d0it);

    for(int z=0; z<ordersSz; z++){char**vec = orders[z];
        ++cnt[mapGet(table, vec[1])][mapGet(dish, vec[2])];
    }
    for(int i=1; i<=t; ++i)
        for(int j=1; j<=d; ++j)
            res[i][j] = to_string(cnt[i][j]);
    int*cols = calloc(t+1, sizeof(int));
    for(int z=0; z<t+1; z++)
        cols[z] = d+1;
    *csz = cols;
    *rsz = t+1;
