...
void sortA_L(int N, ll*a, int*b){
  int i;
  pli*arr = calloc(N, sizeof(pli));
  for(i=(0);i<(N);i++){
    arr[i].first  = a[i];
    arr[i].second = b[i];
  }
  qsort(arr, N, sizeof(pli), cmpP);
  for(i=(0);i<(N);i++){
    a[i] = arr[i].first;
    b[i] = arr[i].second;
  }
}
int my_getchar_unlocked(){
...
void fmalloc(int mem){
  memory = mem;
  data = (ll*)malloc(sizeof(ll)*mem);
}
void ffree(void){
  memory = 0;
  free(data);
...
int main(){
  ...
  for(TEST=(0); TEST<(Lj4PdHRW); TEST++){
    ...
    sortA_L(N, val, ind);
    while(sm){
      while(x < N && R[ind[x]] > sm){
        insert(&s, ind[x++]);
      }
      if(size(&s)==0){
        res1 = kk;
        res2 = 4611686016279904256LL;
        break;
      }
      k = begin(&s);//first
      erase(&s, k);
