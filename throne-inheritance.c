...
int mapEmpty(const msi *karta);
void mapClear(msi *karta);
void mapDestroy(msi *karta);
#endif
...
ThroneInheritance*throneInheritanceCreate(char*kingName){
    ...
    p[0]=-1;
    ch[0].sz=0;
    mapPut(mp, kingName, 0);
    n=1;
    memset(isdie, 0, sizeof(isdie));
    return gptr;
}
void throneInheritanceBirth(ThroneInheritance*obj, char*parentName, char*childName){
    int i, j, k, x;
    x = mapGet(mp, parentName);
    a[n] = strdup(childName);
    p[n] = x;
    ch[n].sz=0;
    pb(ch[x], n);
    mapPut(mp, childName, n);
    n++;
}
...
char**throneInheritanceGetInheritanceOrder(ThroneInheritance*obj, int*rsz){
    asz=0;
    doit(0);
   *rsz=   asz;
    return ans;
