...
int*smallestSufficientTeam(char**req_skills, int req_skillsSz, char***people, int peopleSz, int*people0sz, int*rsz){
    int T=req_skillsSz;
    int N=peopleSz;
    HashMap*M = newHashMap();
    int i,j;
    for(i=0; i<T; i++)
        mapPut(M, req_skills[i], 1<<i);
    for(i=0; i<(1<<T); i++)
        from[i]=-1LL;
    from[0]=0;
    for(i=0; i<(N); i++){
        int mask=0;
        for(int z=0; z<people0sz[i]; z++){char*s = people[i][z];
            mask |= mapGet(M, s);
        }
        for(j=0; j<(1<<T); j++){
            ll ns=j | mask;
