...
long f(char*s){
  long y, m, d, H, M, S;
  sscanf(s, "%ld:%ld:%ld:%ld:%ld:%ld", &y, &m, &d, &H, &M, &S);
  return ((((y*100+m)*100+d)*100+H)*100+M)*100+S;
}
LogSystem* logSystemCreate() {
  a=hashmap_create(_m_int_hash, _m_int_equal);
  return ptr;
}
void logSystemPut(LogSystem* obj, int id, char* timestamp) {
  hashmap_add(a, id, f(timestamp));
}
int* logSystemRetrieve(LogSystem* obj, char* s, char* e, char* gra, int *rSz) {
  long l = f(s), h = f(e);
  long long gran = 1;
  if     (strcmp(gra, "Year"  )==0) 
    gran = 10000000000LL;
  else if(strcmp(gra, "Month" )==0) gran = 100000000;
  else if(strcmp(gra, "Day"   )==0) gran = 1000000;
  else if(strcmp(gra, "Hour"  )==0) gran = 10000;
  else if(strcmp(gra, "Minute")==0) gran = 100;
  else if(strcmp(gra, "Second")==0) gran = 1;
  l = l/gran*gran;
  h = h/gran*gran;
  int*r=NULL;
  hashmap_iter_t it = hashmap_iter_create(a);
  while(hashmap_iter_has_next(it)){
    hmpair*kv=hashmap_iter_next(it);
    if (l <= kv->value/gran*gran && kv->value/gran*gran <= h)
      r = pushback(r, rSz, kv->key);
  }
  return r;
}
