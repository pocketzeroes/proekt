...
char szxBuf[2020 +10];
int N,M;
int lnum0[2020], lnum1[2020], lnumt[2020];
int rnum0[2020], rnum1[2020], rnumt[2020];
int par[2020*2020 + 200],h[2020*2020 + 200];


void clearx(){
 int i;
 for(i=0;i<((N) > (M) ? (N) : (M))+100;++i) {
...
pair calc(int j){
 return newPair(lnum0[j]+rnum0[j], 1 + lnum1[j] + rnum1[j] - lnumt[j] - rnumt[j]);
}
int main(){
 int T, id=1, i;
 T=1;
 while(T--){
  ...
  int ret = 0;
  int nr = 0;
  pair retv = newPair(0,0);
  for (i = 0; i < (M); ++i){
    pair a = calc(i);
    if(a.first + a.second > ret){
      ret = a.first + a.second;
      retv = a;
    }
    else if(a.first + a.second == ret && compP(retv, a)<0){
      retv = a;
    }
  }
  printf("%d %d\n", retv.first, retv.second);
 }
 return 0;
}
