...
#define pb2d(zpv, zvv) zpv.ptr = pushback2d(zpv.ptr, &zpv.sz, zvv)
...
static int intnode_comparator(const void*l, const void*r){
  intnode_t*li =(intnode_t*)l;
  intnode_t*ri =(intnode_t*)r;
int cmpVec(vec*a, vec*b){
  ...
  if(cmpVec(&li->in_val, &ri->in_val)>0)
    return (1);
  if(cmpVec(&li->in_val, &ri->in_val)<0)
    return(-1);
  return 0;
}
void add(Set*s, vec i){
  intnode_t *inp;
...
void split(vec*p, int at, int val, int x, vec2d*ncur, int lasti){
  if(at == p->sz){
    if(val<=x){
      vec nnv=newVec(); copyVec(nnv, nv);
      qsort(nnv.ptr, nnv.sz, sizeof(int), cmp);
      if(!contains(&seen, nnv)){
        pb2(ncur, nnv);
        insert(&seen, nnv);
      }
    }
  ...
  for(int i=0; i<=val && i<=p->ptr[at]; i++){
    if(at>0 && p->ptr[at] == p->ptr[at-1] && i<lasti)
      continue;
    if(i > 1)
      pb(nv, i);
    if(p->ptr[at]-i > 1)
      pb(nv, p->ptr[at]-i);
    split(p, at+1, val-i, x, ncur, i);
    if(i>1)
      nv.sz--;
    if(p->ptr[at]-i > 1)
      nv.sz--;
...
int main(){
  ...
  for(int z=0;z<m;z++)
    scanf("%d", &a[z]);
  vec2d cur=newVec2d();
  vec zzz=newVec(); pb(zzz, n);
  pb2d(cur, zzz);
  insert(&seen, zzz);
  for(int z=0; z<m; z++){int v=a[z];
    vec2d ncur=newVec2d();
    for(int w=0;w<cur.sz;w++){vec s=cur.ptr[w];
      int x = n;
      for(int q=0;q<s.sz;q++){int v=s.ptr[q];
        x -= v;
      }
      r = fmax(r, x);
      split(&s, 0, v, x, &ncur, -1);
    }
    swap(cur, ncur);
  }
  for(int z=0;z<cur.sz;z++){vec s=cur.ptr[z];
    int x=n;
    for(int q=0;q<s.sz;q++){int v=s.ptr[q];
      x -= v;
    }
