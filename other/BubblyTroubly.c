...
void delta(double x1, double y1, double r1, double x2, double y2, double r2, double*lo, double*hi){
    double dx = x2 - x1;
    double dy = y2 - y1;
...
int main(){
    scanf("%d", &n);
    for(int i=0; i<n; ++i){
        scanf("%lf %lf %d %lf %lf", &x[i], &y[i], &z[i], &r[i], &vol[i]);
        if(z[top] < z[i])
            top = i;
    }
    for(int i=0; i<n; ++i){
        int range_tot = 1;
        double ranges[1000][2]; clr(ranges);
        ranges[0][0] =-M_PI; ranges[0][1] = M_PI;
        double cut_range(double lo, double hi){
            double cut = 0;
        ...
        for(int z_drop = z[i] - 1; z_drop >= 0; --z_drop){
            for(int j = 0; j < n; ++j){
                ...
                if(lo < hi)
                    ...
                    cut = cut_range(-M_PI, hi) + cut_range(lo, M_PI);
                flow[i][j] = cut / (2 * M_PI);
            }
        }
    }
