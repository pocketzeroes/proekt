...
int mapPut(hashmap_t map, pair key, pair value){
  if (!map)
      return -1;
...
pair mapGet(hashmap_t map, pair key){
  if (!map)
  ...
  if (hash_code < 0)
      hash_code += map->capacity;
  struct hashmap_pair *hpair = _hashmap_list_find(map, map->lists[hash_code], key);
  if (hpair == NULL)
      return(pair){0,0};
  else
      return hpair->value;
...
void solve(){
  ...
  for(int i=1; i<MAXN; ++i){
    powh1[i] = (ll)powh1[i - 1] * HASH1 % MOD;
    powh2[i] = (ll)powh2[i - 1] * HASH2 % MOD;
  }
  scanf("%d", &n);
  for(int j = 0; j < n; ++j){
    scanf("%s",s);
    ...
    int best = 0;
    for (int i = 0; i < m; ++i) {
      h_back1  = ((ll)h_back1 * HASH1 + (int)(s[m - 1 - i])) % MOD;
      h_front1 = (h_front1 + (ll)powh1[i] * (int)(s[i])) % MOD;
      h_back2  = ((ll)h_back2 * HASH2 + (int)(s[m - 1 - i])) % MOD;
      h_front2 = (h_front2 + (ll)powh2[i] * (int)(s[i])) % MOD;
      if (h_front1 == h_back1 && h_front2 == h_back2 && best < mapGet(dp, (pair){h_front1, h_front2}).value){
        best = mapGet(dp, (pair){h_front1, h_front2}).value;
        previous[j] = mapGet(dp, (pair){h_front1, h_front2}).idx;
      }
    }
    mapPut(dp, (pair){h_front1, h_front2}, (pair){best + 1, j});
    if(best + 1 > res){
      res = best + 1;
      res_idx = j;
    }    
...
int main(){
  dp = newMap(_m_int_hash, _m_int_equal);
  solve();
  return 0;
}
