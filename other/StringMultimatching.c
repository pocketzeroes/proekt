...
void add(char*str){
    ...
    dict[v] = v;
    pbp(ind[v], newpair(++num, strlen(str)));
}
void push_links(){
    link[0] = -1;
    push(q, 0);
    while(size(q)){
        ...
        void doit(kvp*it){
            ...
            int  u = it->val,
                 j = link[v];
            while(j != -1 && !mapHas(to[j], c))
                j = link[j];
            if(j != -1){
                link[u] = mapGet(to[j], c);
                if(!dict[u])
                    dict[u] = dict[link[u]];
            }
            push(q, u);
        }
        mapForEach(to[v], doit);
    }
}
void process(int pos, int cur){
    cur = dict[cur];
    while(cur){
        for(int z=0;z<ind[cur].sz;z++){pair a = ind[cur].ptr[z];
            pb(oc[a.first], pos-a.second+1);
        }
        cur = dict[link[cur]];
    }
}
int nex(int pos, int cur, char c){
    while(cur != -1 && !mapHas(to[cur], c))
        cur =link[cur];
    if(cur == -1)
        cur = 0;
    else
...
int main(){
    while(scanf("%d", &n)==1){
        ...
        for(int i=0; t[i]; i++)
            cur = nex(i, cur, t[i]);
        for(int i=1; i<n+1; i++){
            for(int z=0;z<oc[i].sz;z++){int j = oc[i].ptr[z];
                printf("%d ", j);
            }
            puts("");
        }
