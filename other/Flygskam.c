...
pil newpil(int a, ld b){
  return(pil){a,b};
}

#define pbp(zpv, zvv) zpv.ptr = pushbackP(zpv.ptr, &zpv.sz, zvv)
#define resizeArray(ptr, type, size) ((type*)realloc(ptr, (size) * sizeof(type)))
pil*pushbackP(pil*array, int *size, pil value){
  pil*output = resizeArray(array, pil, *size + 1);
  output[(*size)++] = value;
  return output;
}
typedef struct{
...
int main(){
    ...
    for(int i=0; i<n; i++){
        ...
        lat *= M_PI / 180;
        lng *= M_PI / 180;
        nodes[i].x = cosl(lng) * cosl(lat);
        nodes[i].y = sinl(lat);
        nodes[i].z = sinl(lng) * cosl(lat);
        nodes[i].minDist = INFINITY;
        mapPut(mp, name, i);
    }
    for(int i = 0; i < m; i++){
        char*a=getstr();
        ...
        ld dist = acosl(dot) * 6381 + 100;
        pbp(nodes[ai].edges, newpil(bi, dist));
        pbp(nodes[bi].edges, newpil(ai, dist));
    }
    int startI = mapGet(mp, startS);
    int endI   = mapGet(mp, endS  );
    PriorityQueue*pq = newPriorityQueue(cmpP);// of pli . pli{ld, int}
    push(pq, mkpli(0, startI));
    nodes[startI].minDist = 0;
    while(!empty(pq)){
        pli*tq=top(pq); int cur = tq->second; pop(pq);
        if(cur == endI){
            printf("%.15Lf\n", nodes[endI].minDist);
            return 0;
        }
        for(int z=0;z<nodes[cur].edges.sz;z++){pil edge = nodes[cur].edges.ptr[z];
            ld ndst = nodes[cur].minDist + edge.second;
            if(nodes[edge.first].minDist > ndst){
                nodes[edge.first].minDist = ndst;
                push(pq, mkpli(-ndst, edge.first));
            }
        }
    }
    puts("-1");
