...
void SPFA(int s){
    dis[s]=0;
    Queue*table = newQueue();
    push(table, s);
    bool inTable[100001]={0};
    inTable[s]=1;
    int u, v;
    while(!empty(table)){
        u = front(table); pop(table);
        inTable[u] = false;
        for(int z=0;z<list[u].sz;z++){pair pr=list[u].ptr[z];
            v = pr.first;
            if( dis[v] > dis[u]+pr.second){
                dis[v] = dis[u]+pr.second;
                if(!inTable[v]){
                    push(table, v);
                    inTable[v]=true;
                }
...
int main(){
    ...
    int q, s;
    int u, v, w;
    while(scanf("%d %d %d %d", &n, &m, &q, &s)==4 && n){
        if(tc++>0)
            puts("");
        dis = calloc(n, sizeof(int));
        for(int i=0; i<n; ++i){
            dis [i] = 100000000;
            list[i].sz=0;
        }
