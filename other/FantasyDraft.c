...
int main(){
    ...
    scanf("%d %d", &n, &k);
    char**prefs[n];
    int   sizes[n]; clr(sizes);
    for(int i=0; i<n; ++i){
        int gsz;
        scanf("%d", &gsz);
        sizes[i] = gsz;
        prefs[i] = calloc(gsz, sizeof(char*));
        for(int j=0; j<gsz; ++j){
            prefs[i][j] = getstr();
        }
    }
    ...
    int pref_ptrs[n]; clr(pref_ptrs);
    int name_ptr = 0;
    msi*taken = newMap();//setstr
    vecs ans[n]; clr(ans);
    for(int round=0; round<k; ++round){
        for(int i=0; i<n; ++i){
            while(pref_ptrs[i] < sizes[i] && mapHas(taken, prefs[i][pref_ptrs[i]]))
                ++pref_ptrs[i];
            while(name_ptr < p && mapHas(taken, names[name_ptr]))
                ++name_ptr;
            if(pref_ptrs[i] < sizes[i])
                pb(ans[i], prefs[i][pref_ptrs[i]++]);
            else
                pb(ans[i], names[name_ptr++]);
            mapPut(taken, ans[i].ptr[ans[i].sz-1], 5);
        }
    }
    for(int i=0; i<n; ++i){
        for(int z=0;z<ans[i].sz;z++){char*player = ans[i].ptr[z];
            printf("%s ", player);
        }
        puts("");
    }
