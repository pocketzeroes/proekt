...
int main(){
    while(1){
        ...
        Set walls ; newSet(&walls );  //of pair
        Set wet   ; newSet(&wet   );
        Set fringe; newSet(&fringe);
        for(int j=0; j<L; j++){
            pair leak;
            scanf("%d %d", &leak.first, &leak.second);
            insert(&wet, leak);
            insert(&fringe, leak);
        }
        for(int j=0; j<W; j++){
            ...
            if (dy != 0) 
                dy = dy / abs(dy);
            pair wall = (pair){x1, y1};
            insert(&walls, wall);
            while(wall.first != x2 || wall.second != y2){
                wall.first  += dx;
                wall.second += dy;
                insert(&walls, wall);
            }
        ...
        for(int step=1; step<T; step++){
            Set newest; newSet(&newest);//of pair
            for(intnode_t*inp = avl_first(&fringe); inp != NULL; inp = AVL_NEXT(&fringe, inp)){pair it = inp->in_val;
                pair neighbor[4] ={
                ...
                for(int d=0; d < 4; d++){
                    if(neighbor[d].first > 0 && neighbor[d].first  <= X &&
                       neighbor[d].second> 0 && neighbor[d].second <= Y &&
                        count(&walls,   neighbor[d]) == 0 &&
                        count(&wet,     neighbor[d]) == 0){
                        insert(&wet,    neighbor[d]);
                        insert(&newest, neighbor[d]);
                    }
                }
            }
            avl_swap(&fringe, &newest);
            if(empty(&fringe))
                break;
        }
        printf("%d\n", size(&wet));
