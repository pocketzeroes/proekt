...
void calculateDistances(){
    ...
    for(intnode_t*inp = avl_first(&shrines); inp != NULL && j<S; inp = AVL_NEXT(&shrines, inp), ++j){int it = inp->in_val;
        int sep = it-prev;
        prev = it;
        dist[(j-1+S)%S] = 2000*sin(M_PI*sep/N);
    }
}
...
int main(){
    while(1){
        ...
        calculateDistances();
        double lower = 2000.0;
        double upper = 2000.0 + 2*M_PI*2000.0;
