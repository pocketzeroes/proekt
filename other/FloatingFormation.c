...
int cmp(const void*pa, const void*pb){
  int i = *((int*)pa);
  int j = *((int*)pb);
  return(MaxBranch[i] < MaxBranch[j])?-1:1;
}
void sink (int i, int brlen){
  MaxBranch[i] = fmax(MaxBranch[i], brlen);
  if(size(&Adj[i]) == 1){
    InDanger[i] = true;
    intnode_t*beg = avl_first(&Adj[i]);
    int j = beg->in_val;
    Child[j].ptr = pushback(Child[j].ptr, &Child[j].sz, i);
    erase(&Adj[j], i);
    sink(j, MaxBranch[i]+1);
  }
}
int main(){
  ...
  for(run = 0; run < runs; run++){
    ...
    for(i = 0; i < n; i++)
      if(size(&Adj[i]) == 1 && MaxBranch[i] == 0)
        sink(i, 1);
    willsink = 0;
    for(i = 0; i < n; i++)
      if(InDanger[i])
        Idx[willsink++] = i;
    qsort(Idx, willsink, sizeof(int), cmp);
    for(q = willsink-1; q >= 0 && k > 0; q--){
      i = Idx[q];
      if(MaxBranch[i] == 0)
        continue;
      k--;
      willsink -= b = MaxBranch[i];
      for(b--; true; b--){
        MaxBranch[i] = 0;
        if (b == 0)
          break;
        for(j = 0; MaxBranch[Child[i].ptr[j]] != b; j++);
        i = Child[i].ptr[j];
      }
    }
    printf("%d\n", willsink);
