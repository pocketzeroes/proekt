...
int    qempty(Queue *Q);
int    pushq(Queue *Q, QueueElementType e);
QueueElementType popq(Queue *Q);
...
int pushq(Queue *Q, QueueElementType e)
{
  if (Q->NumElements == Q->Capacity)
  {
    ...
    if (newE == NULL)
    {
      printf("\n**Error in pushq: malloc failed to allocate\n\n");
      exit(-1);
    }
    int  i = Q->Front;
    int  j = 0;
...
ll*dijkstra(vecp*adj, int s){
  ll*dist =(ll*)calloc(edges_sz, sizeof(ll)); fill(dist, edges_sz, oo);
  dist[s] = 0;
  Set q; newSet(&q); // of pli
  insert(&q, newpli(0,s));
  while(size(&q)){
    pli beginq = begin(&q);
    int i = beginq.second;
    erase(&q, beginq);
    for(int z=0;z<adj[i].sz;z++){Pair e = adj[i].ptr[z];
      int j   = e.first,
          len = e.second;
      if(dist[j] <= dist[i] + len)
        continue;
      erase(&q, newpli(dist[j],j));
      dist[j] = dist[i] + len;
      insert(&q, newpli(dist[j],j));
    }
  }
  return dist;
}
int main(){
  ...
  scanf("%d %d", &n, &m);
  vecp adj    [n]; clr(adj    );
  vecp rev_adj[n]; clr(rev_adj);
  edges_sz = n;
  while(m--){
    int u, v, w;
    scanf("%d %d %d", &u, &v, &w);
    pbp(adj    [u-1], newpair(v-1, w));
    pbp(rev_adj[v-1], newpair(u-1, w));
  }
  pbp(adj[0]    ,newpair(0,0));
  pbp(rev_adj[0],newpair(0,0));
  ll*from_start = dijkstra(adj, 0);
  ll*to_finish  = dijkstra(rev_adj, n-1);
  ll kmin = -1,
     kmax =(ll)1e16;
  while(kmax-kmin > 1){
    ll k = (kmin+kmax)/2;
    bool ok[n];   clr(ok);
    ll latest[n]; fill(latest, n, -oo);
    Queue*q = newQueue();
    for(int i=0; i<n; i++){
    ...
    vecp ok_adj[n]; clr(ok_adj);
    int  indeg [n]; clr(indeg );
    for(int i=0; i<n; i++){
      if(ok[i]){
        for(int z=0;z<adj[i].sz;z++){Pair e = adj[i].ptr[z];
          int j   = e.first,
    ...
    while(sizeq(q)){
      int i = front(q); popq(q);
      for(int z=0;z<ok_adj[i].sz;z++){Pair e = ok_adj[i].ptr[z];
        int j   = e.first,
            len = e.second;
        if(ok[j]){
          latest[j] = fmaxl(latest[j], latest[i] + len);
          if(--indeg[j] == 0)
            pushq(q, j);
        }
      }
    }
    bool possible = latest[n-1] >= b || accumulate(indeg, n) > 0;
    if(possible)
      kmax = k;
    else
      kmin = k;
