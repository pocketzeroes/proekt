...
int comp(const void*pa, const void*pb){
    int*a=(int*)pa;
    int*b=(int*)pb;
    return(W.ptr[*a] < W.ptr[*b])?-1:1;
}
void rm(){
    pair el = last(&S);
    int w = el.first;
    sum -= w;
    min_rm = fmin(min_rm, w);
    if(min_rm == 0)
       min_rm =  w;
    erase(&S, el);
}
int main(){
    ...
    for(int rev=0; rev<2; rev++){
        ...
        for(int i2=0; i2<n; i2++){
            ...
            if(s[i] != '0'){
                if(rev == 0) voters++;
                if(B.ptr[i] > 0) last_positive = voters;
                if(B.ptr[i] ==0) last_zero = voters;
                if(rev == 1) voters++;
            }
            else{
                P.ptr[i] = fmin(P.ptr[i], abs(voters - last_positive));
                Z.ptr[i] = fmin(Z.ptr[i], abs(voters - last_zero));
            }
        }
    }
    for(int i=0; i<n; ++i){
        if(s[i] == '0'){
            if(B.ptr[i] < 0){
                if(Z.ptr[i] < P.ptr[i] - Z.ptr[i]){
                    insert(&S, newpair(Z.ptr[i], i));
                    insert(&S, newpair(P.ptr[i] - Z.ptr[i], i));
                    sum += P.ptr[i];
                }
                else{
                    pb(ind, ind.sz);
    ...
    if(size(&S) == need)
        ans = sum;
    qsort(ind.ptr, ind.sz, sizeof(int), comp);
    int max_second = -n;
    for(int j = 0; j<ind.sz; j++){
        int i = ind.ptr[j];
        sum += W.ptr[i];
        int i2 = I.ptr[i];
        max_second = fmax(max_second, P.ptr[i2]-Z.ptr[i2]);
        if(isin(&S, newpair(Z.ptr[i2],i2))){
            erase(&S, newpair(Z.ptr[i2],i2));
            sum -= Z.ptr[i2];
        }
        while(size(&S) > fmax(0, need - 2*j - 2))
            rm();
        if(size(&S) + 2*j + 2 >= need){
            ans = fminl(ans, sum);
            if(min_rm > 0)
                ans = fminl(ans, sum + min_rm - max_second);
            if(size(&S) + 2*j + 2 > need)
                ans = fminl(ans, sum - max_second);
        }
    }
    printf("%lld\n", ans);
    return 0;
