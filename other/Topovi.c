...
void moveRook(int r, int c, int val){
  int rxorr = mapGet(rxor, r);
  int cxorc = mapGet(cxor, c);
  sol -= n - mapGet(ccnt, rxorr);
  sol -= n - mapGet(rcnt, cxorc);
  if(rxorr != cxorc)
    sol += 1;
  dec(rcnt, rxorr);
  mapPut(rxor, r, rxorr^val);
  rxorr   ^= val;
  inc(rcnt, rxorr);
  dec(ccnt, cxorc);
  mapPut(cxor, c, cxorc^val);
  cxorc   ^= val;
  inc(ccnt, cxorc);
  sol += n - mapGet(ccnt, rxorr);
  sol += n - mapGet(rcnt, cxorc);
  if(rxorr != cxorc)
    sol -= 1;
  ll erc = Encode(r, c);
  mapPut(rook, erc, mapGet(rook, erc)^val);
}
void init(){
  scanf("%d %d %d", &n, &k, &q);
  mapPut(rcnt, 0, n);
...
void solve(){
  while(q-- > 0){
    ...
    --r1, --c1, --r2, --c2;
    int rookValue = mapGet(rook, Encode(r1, c1));
