...
int main(){
    ...
    fill(rights, n,   -1);
    best = 0;
    paper = newmat(n, m);//of char
    getchar();
    for(int i=0; i<n; i++){
        for( int j=0; j<m; j++ ){
            paper[i][j] = getchar();
            if( paper[i][j]=='#'){
                marks.ptr = pushbackPt(marks.ptr, &marks.sz, newpt(i, j));
    ...
    if( (best&1)==1 )
        ++bestpossible;
    for( int i=0; i<n; i++ )
        if( lefts[i]<1000 && lefts[i]>=0 )
            for( int j=i+1; j<n; j++ )
                if( lefts[j]<1000 && lefts[j]>=0 )
                    test( j-i, lefts[j]-lefts[i] );
    for( int i=0; i<n; i++ )
        if( rights[i]<1000 && rights[i]>=0 )
            for( int j=i+1; j<n; j++ )
                if( rights[j]<1000 && rights[j]>=0 )
                    test( j-i, rights[j]-rights[i] );
    for( int i=0; i<m; i++ )
        if( uppers[i]<1000 && uppers[i]>=0 )
            for( int j=i+1; j<m; j++ )
                if( uppers[j]<1000 && uppers[j]>=0 )
                    test( uppers[j]-uppers[i], j-i );
    for( int i=0; i<m; i++ )
        if( lowers[i]<1000 && lowers[i]>=0 )
            for( int j=i+1; j<m; j++ )
                if( lowers[j]<1000 && lowers[j]>=0 )
                    test( lowers[j]-lowers[i], j-i );
    printf("%d\n", best);
    return 0;
}

void test(int di, int dj){
    if(best>bestpossible && !contains(&seen, pack(di, dj))){
        insert(&seen, pack( di,  dj));
        insert(&seen, pack(-di, -dj));
        int nubs = 0;
        for(int z=0; z<marks.sz; z++){pt mark = marks.ptr[z];
            int ii = mark.i-di;
            int jj = mark.j-dj;
            if(ii<0 || ii>=n || jj<0 || jj>=m || paper[ii][jj]=='.'){
                int count = 0;
                for(ii=mark.i, jj=mark.j; ii>=0 && ii<n && jj>=0 && jj<m && paper[ii][jj]=='#'; ii+=di, jj+=dj)
                    ++count;
                if(count==1){
                    nubs = 1000000;
                    break;
