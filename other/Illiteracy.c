...
int main(){
   ...
   for(int i=0; i<q.sz; i++){
      int d = mapGet(dist, q.ptr[i]);
      for (int j=0; j< strlen(q.ptr[i]); j++){
         char*s = strdup(q.ptr[i]);
         ...
         switch(s[j]){
          ...
          case 'B':
            ...
            break ;
          case 'C':
            s[strlens-1-j] = rot(s[strlens-1-j]) ;
            break ;
          case 'D':
            if (j + j < strlens)
               for (int k=0; k<j; k++)
          ...
          case 'E':
            if (j + j < strlens) {
               ...
               if (j + 1 < strlens) {
                  s[strlens-1] = rot(s[strlens-1]) ;
                  s[2*j-strlens+1] = rot(s[2*j-strlens+1]) ;
               }
            }
            break ;
          case 'F':
            if (j & 1)
               ...
               s[(j+strlens)>>1] = rot(s[(j+strlens)>>1]) ;
            break ;
         }
         if(strcmp(s, dst)==0){
            printf("%d\n", (d + 1) );
