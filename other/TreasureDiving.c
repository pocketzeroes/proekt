...
void dijkstra(int from){
  dist = calloc(numNodes, sizeof(int)); fill(dist, numNodes, INT_MAX/2);
  dist[from] = 0;
  Set pq; newSet(&pq);//of pair
  insert(&pq, newpair(dist[from], from));
  while(!empty(&pq)){
    int node = begin(&pq).second;
    erase(&pq, begin(&pq));
    for(int i=0; i<adjList[node].sz; i++){
      int next = adjList [node].ptr[i];
      int cost = costList[node].ptr[i];
      if(dist[node] + cost >= dist[next])
        continue;
      if(dist[next] < INT_MAX/2)
        erase(&pq, newpair(dist[next], next));
      dist[next] = dist[node] + cost;
      insert(&pq, newpair(dist[next], next));
    }
  }
}
int tsp(int usedNodes){
  for(int i=0; i<pow2[usedNodes]; i++)
    for(int j=0; j<usedNodes; j++)
      opt[i][j] = INT_MAX/2;
  opt[1][0] = 0;
  for(int i=1; i<pow2[usedNodes]; i++)
    for(int j=0; j<usedNodes; j++){
      if(opt[i][j] == INT_MAX)
        continue;
    for(int k=0; k<usedNodes; k++){
      if((i >> k) & 1 || tspMatrix[j][k] == INT_MAX/2)
        continue;
  ...
  for(int i=1; i<usedNodes; i++){
    if(opt[pow2[usedNodes]-1][i] == INT_MAX || tspMatrix[i][0] == INT_MAX)
      continue;
...
int main(){
  ...
  while(EOF != scanf("%d %d", &numNodes, &numEdges)){
    ...
    for(int i=0; i<numIdols+1; i++){
      dijkstra(idols.ptr[i]);
      for(int j=0; j<numIdols+1; j++)
        distMatrix[i][j] = dist[idols.ptr[j]];
    }
    scanf("%d", &air);
    int best = 0;
    pow2[0] = 1;
    for(int i=1; i<numIdols + 2; i++)
      pow2[i] = 2 * pow2[i-1];
    for(int i=1; i<pow2[numIdols + 1]; i++){
      ...
      if(tsp(usedNodes) <= air)
        best = fmax(best, usedNodes - 1);
    }
    printf("%d\n", best);
  }
  return 0;
