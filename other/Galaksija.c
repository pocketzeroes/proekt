...
void DSU(int n){
  pai  = calloc(n+1, sizeof(int));
  peso = calloc(n+1, sizeof(int));
  pairs = 0;
  for(int i=1; i<=n; i++){
    pai[i] = i, peso[i] = 1;
    mapPut(qtd[i], 0, 1);
...
void Merge(int x, int y, int w){
  void doit(kvp*v){
    pairs += 1ll * mapGet(qtd[x], v->key^w)*v->val;
  }
  mapForEach(qtd[y], doit);
  void doit2(kvp*v){
    mapPut(qtd[x], v->key^w, mapGet(qtd[x], v->key^w) + v->val);
  }
  mapForEach(qtd[y], doit2);

  for(int z=0;z<elements[y].sz;z++){int v = elements[y].ptr[z];
    pb(elements[x], v);
    path[v] ^= w;
  }
  mapClear(qtd[y]);
