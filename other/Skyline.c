...
static int intnode_comparator(const void *l, const void *r){
    ...
    return cmpDbl(li->in_val, ri->in_val);
}
void add(Set*s, double i){
    intnode_t *inp;
...
int main(){
    ...
    scanf("%d", &N);
    Set xcoords; newSet(&xcoords); //of double
    insert(&xcoords, 0.);
    insert(&xcoords, 10000.);
    for(int i=0; i<N; i++){
        ...
        for(int j=0; j<i; j++){
            ...
            if (d0 != 0){
                ...
                if (fa>0 && fa<d0 && fb>0 && fb<d0)
                    insert(&xcoords, fa*v1x/d0+x1[i]);
            }
        }
    }
    double last=0.0;
    for(intnode_t*inp = avl_first(&xcoords); inp != NULL; inp = AVL_NEXT(&xcoords, inp)){ double si= inp->in_val;
        double current = si;
        double covered=0.0;
        for(int i=0; i<N; i++){
