...
int doit(){
	...
	if (c == '(') {
		...
		right_id = doit();
		assert(c == ')');
		if(mapHas(nodes, (left_id << 48) + (right_id << 32) + code)){
			--number_of_nodes;
			codes[curn] = - mapGet(nodes, (left_id<<48) + (right_id << 32) + code);
		}
		else{
			mapPut(nodes, (left_id << 48) + (right_id << 32) + code, cur_node);
			codes[curn] = code;
		}
		c = getchar();
	}
		...
		if(mapHas(nodes, code)){
			--number_of_nodes;
			codes[curn] = - mapGet(nodes, code);
		}
		else{
			mapPut(nodes, code, cur_node);
			codes[curn] = code;
		}
	}
	return mapGet(nodes, (left_id << 48) + (right_id << 32) + code);
}
...
int main(){
  nodes=newMap(_m_int_hash, _m_int_equal);
	int tc;
	scanf("%d", &tc);
	assert(tc >= 1 && tc <= 200);
	while((c = getchar()) != '\n');
