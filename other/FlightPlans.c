...
int main(){
    ...
    int dist[N]; fill(dist, N, -1);
    Queue*q = newQueue();
    push(q, s);
    dist[s] = 0;
    Set remaining; newSet(&remaining);
    for(int i=0; i<N; ++i)
        insert(&remaining, i);
    erase(&remaining, s);
    while(!empty(q)){
        ...
        int d = dist[x];
        if(type[x] == 'N'){
            for(intnode_t*inp = avl_first(&ed[x]); inp != NULL; inp = AVL_NEXT(&ed[x], inp)){ int y = inp->in_val;
                if(dist[y] == -1){
                    dist[y] = d + 1;
                    erase(&remaining, y);
                    push(q, y);
            ...
            vec er=newVec();
            for(intnode_t*inp = avl_first(&remaining); inp != NULL; inp = AVL_NEXT(&remaining, inp)){ int y = inp->in_val;
                if(!count(&ed[x], y)){
                    dist[y] = d + 1;
                    pb(er, y);
                    push(q, y);
            ...
            for(int z=0;z<er.sz;z++){int y = er.ptr[z];
                erase(&remaining, y);
            }
        }
    }
