...
int main(){
    mp = newMap();
    int n = 0,i,j,ask,tot;
    while((tmp=getstr())!=NULL){
        if(strcmp(tmp, "<text>")==0){
            j = n%7;
            for(i=0; i<v[j].sz; i++)
                mapPut(mp, v[j].ptr[i], mapGet(mp, v[j].ptr[i])-1);
            v[j].sz=0;
            while((tmp=getstr())!=NULL){
                if(strcmp(tmp, "</text>")==0)
                    break;
                if(strlen(tmp) >= 4){
                    mapPut(mp, tmp, mapGet(mp, tmp)+1);
                    v[j].ptr = pushbackS(v[j].ptr, &v[j].sz, tmp);
                }
            }
            n++;
        }
            ...
            tot = 0;
            void doit(kvp*it){
                nima[tot++] = newcao(it->key, it->val);
            }
            mapForEach(mp, doit);
            qsort(nima, tot, sizeof(cao), cmp);
            printf("<top %d>\n", ask);
            for(i=0,j=0; i<ask; i++)
                printf("%s %d\n", nima[i].word, nima[i].cnt);
            for(j = i,i--; j<tot&&nima[j].cnt==nima[i].cnt; j++)
                 printf("%s %d\n", nima[j].word, nima[j].cnt);
            puts("</top>");
        }
    }
    return 0;
