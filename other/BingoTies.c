...
int main(){
    ...
    for(int last = 1; last <= 3000; ++last){
        ...
        for (int i = 0; i < found_tot; ++i){
            #define data_i data[found[i]]
            for (int j = i + 1; j < found_tot; ++j){
                #define data_j data[found[j]]
                Set s;newSet(&s);
                insert(&s, data_i[found_x[i]][0]);
                insert(&s, data_i[found_x[i]][1]);
                insert(&s, data_i[found_x[i]][2]);
                insert(&s, data_i[found_x[i]][3]);
                insert(&s, data_i[found_x[i]][4]);
                insert(&s, data_j[found_x[j]][0]);
                insert(&s, data_j[found_x[j]][1]);
                insert(&s, data_j[found_x[j]][2]);
                insert(&s, data_j[found_x[j]][3]);
                insert(&s, data_j[found_x[j]][4]);
                erase (&s, last);
                bool fail = false;
                for (int k = 0; k < n; ++k) {
                    if (k == found[i]) 
                    ...
                    for (int x = 0; x < 5; ++x) {
                        int match = 0;
                        for (int y = 0; y < 5; ++y)
                            match += exists(&s, data[k][x][y]);
                        fail |= match == 5;
                    }
                }
                if(!fail)
                    best = minP(best, (pair){found[i], found[j]});
                cleanup(&s);
            }
        }
    }
    if (best.first < n)
        printf("%d %d\n", best.first+1, best.second+1);
    else
        puts("no ties");
    return 0;
}
