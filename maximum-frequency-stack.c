...
FreqStack*freqStackCreate(){
    cnt = hashmap_create(_m_int_hash, _m_int_equal);
    h = 0;
    return gptr;
}
void freqStackPush(FreqStack*obj, int x){
    int new_cnt = hashmap_get_value(cnt, x) + 1;
    freq[new_cnt].ptr = pushback(freq[new_cnt].ptr, &freq[new_cnt].sz, x);
    hashmap_add(cnt, x, new_cnt);
    h = max(h, new_cnt);
}
int freqStackPop(FreqStack*obj){
    int x = freq[h].ptr[freq[h].sz-1]; freq[h].sz--;
    hashmap_add(cnt, x, hashmap_get_value(cnt, x)-1);
    if(freq[h].sz == 0)
        h--;
    return x;
}
