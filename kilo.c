...
struct editorSyntax HLDB[] = {
    {
        C_HL_extensions, C_HL_keywords, "//","\057*","*/", HL_HIGHLIGHT_STRINGS | HL_HIGHLIGHT_NUMBERS
    }
};
...
void editorRowBspChar(erow *row, int at) {
    at++;
    if (row->size <= at)
      return;
    memmove(row->chars+at, row->chars+at+1, row->size-at);
...
void editorBspChar() {
    ...
    if (filecol == 0) {
        ...
        editorRowBspChar(row,filecol-1);
        if (E.cx == 0 && E.coloff)  E.coloff--;
        else                        E.cx--;
    }
    if (row) editorUpdateRow(row);
...
void editorMoveCursor(int key) {
    int filerow = E.rowoff+E.cy; int filecol = E.coloff+E.cx; int rowlen;
    erow *row = (filerow >= E.numrows) ? NULL : &E.row[filerow];
    switch(key) {
        case ARROW_LEFT:{
            if (E.cx == 0) {
    ...
    filerow = E.rowoff+E.cy;  filecol = E.coloff+E.cx;
...
void editorProcessKeypress(int fd) {
    ...
    switch(c) {
        ...
        case CTRL_L:
        case ESC:{
          editorSetStatusMessage("Do you want to quit?");
        }   break;
        default:{
            editorInsertChar(c);
        }   break;
