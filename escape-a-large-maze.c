...
bool isEscapePossible(int**blocked, int blockedSz, int*blocked0sz, int*source, int sourceSz, int*target, int targetSz){
  int D[4][2] = { { 1, 0 }, { -1, 0 }, { 0, 1 }, { 0, -1 } };
  int NR = 1000000;
  int NC = 1000000;
  Set block; newSet(&block);//of ll
  for(int z=0;z<blockedSz;z++){int*p = blocked[z];
    insert(&block, Encode(p[0], p[1]));
  }
  Set vis; newSet(&vis);// of ll
  Queue*Q = newQueue();// of pair
  pair src;
  src.first =source[0];
  src.second=source[1];
  push(Q, src);
  insert(&vis, Encode(source[0], source[1]));
  while(size(Q) > 0 && ssize(&vis) < 20000){
    pair p = front(Q); pop(Q);
    if(p.first == target[0] && p.second == target[1])
      return true;
    for(int i = 0; i < 4; i++){
      int r = p.first  + D[i][0];
      int c = p.second + D[i][1];
      if (0 <= r && r < NR && 0 <= c && c < NC){
        ll e = Encode(r, c);
        if(!Contains(&vis, e) && !Contains(&block, e)){
          insert(&vis, e);
          push(Q, newPair(r, c));
        }
      }
