...
int eatenApples(int*apples, int applesSz, int*days, int daysSz){
    int D = 0;
    int ans = 0;
    PriorityQueue*Q=newPriorityQueue(cmpP);//of pair
    for(int i=0, i_len=(applesSz); i<i_len; ++i){
        D = i;
        if(apples[i]){
            push(Q, mkpair(-(i + days[i]), apples[i]));
        }
        while(!empty(Q) && -((pair*)Q->info[0])->first <= D){
            pop(Q);
        }
        if(!empty(Q)){
            pair*tq = top(Q);
            ...
            ans++;
            if(p.second)
                push(Q, mkpair(p.first, p.second));
        }
    }
    while(!empty(Q)){
        D++;
