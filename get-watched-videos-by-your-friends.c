...
char**watchedVideosByFriends(char***watchedVideos, int watchedVideosSz, int*watchedVideos0sz, int**friends, int friendsSz, int*friends0sz, int id, int level, int*rsz){
    cnt = newMap();
    clr(G);
    clr(d);
    int n = watchedVideosSz;
    for(int i=0; i<n; i++){
        for(int z=0; z<friends0sz[i]; z++){int j = friends[i][z];
            G[i][j] = G[j][i] = 1;
        }
    }
    Queue*q = newQueue();
    push(q, id);
    bool vis[friendsSz]; clr(vis);
    vis[id] = true;
    while(!empty(q)){
    ...
    for(int i=0; i<n; i++){
        if(d[i]!=level)
            continue;
        for(int z=0;z<watchedVideos0sz[i];z++){char*s=watchedVideos[i][z];
            if(mapGet(cnt, s) == 0)
                pbs(ans, s);
            inc(cnt, s);
        }
    }
    qsort(ans.ptr, ans.sz, sizeof(char*), cmp);
   *rsz =  ans.sz;
    return ans.ptr;
}
