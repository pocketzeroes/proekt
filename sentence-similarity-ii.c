...
bool areSentencesSimilarTwo(char**words1, int words1sz, char**words2, int words2sz, char***pairs, int pairsSz, int pairs0sz){
    ...
    for(int z=0; z<pairsSz; z++){char**p = pairs[z];
        if(!mapContainsKey(index, p[0]))mapPut(index, p[0], count++);
        if(!mapContainsKey(index, p[1]))mapPut(index, p[1], count++);
        UnioN( mapGet(index, p[0]), mapGet(index, p[1]));
    }
    for(int i = 0; i < words1sz; ++i){
        char*w1 = words1[i];
        char*w2 = words2[i];
        if(strcmp(w1, w2)==0)
            continue;
        if(!mapContainsKey(index, w1) || !mapContainsKey(index, w2) ||
            find( mapGet(index, w1)) != find( mapGet(index, w2)))
            return false;
    }
    return true;
}
