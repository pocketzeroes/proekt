...
#define pt point
#define pbp(zpv, zvv) zpv.ptr = pushbackPt(zpv.ptr, &zpv.sz, zvv)
...
vecpt cells[HMax];
point*unknown[NMax*NMax];


void union_(int x1, int y1, int x2, int y2, int h);
...
int main(){
  ...
  for(int i=1; i<=N; i++){
    for(int j=1; j<=M; j++){
      scanf("%d", &e[i][j]);
      pbp(cells[e[i][j]], newpoint(i, j));
      c[i][j] = cc;
      point*np = calloc(1, sizeof(point));
      np->x = i;
      np->y = j;
      DL_APPEND(unknown[cc], np);
      t[cc] = cc;
      cc++;
    }
  }
  for(int h=HMax-1; h>=0; h--){
    for(int i=0; i<cells[h].sz; i++){point p = cells[h].ptr[i];
      for(int d=0; d<4; d++){
        if(e[p.x][p.y] <= e[p.x+dx[d]][p.y+dy[d]]){
          union_(p.x, p.y, p.x+dx[d], p.y+dy[d], e[p.x][p.y]);
        }
      }
    }
  }
  for(int i=1; i<=N; i++){
    for(int j=1; j<=M; j++){
      int c = find_(i, j);
      point*pit, *tmp;
      DL_FOREACH(unknown[c], pit){
        p[pit->x][pit->y] = e[pit->x][pit->y];
      }
      DL_FOREACH_SAFE(unknown[c], pit, tmp){ // clear
        DL_DELETE(unknown[c], pit);          // clear
        free(pit);                           // clear
      }                                      // clear
    }
  }
...
void union_(int x1, int y1, int x2, int y2, int h){
  ...
  if(highest(c1) < highest(c2)){
    union_(x2, y2, x1, y1, h);
    return;
  }
  t[c2] = c1;
  if(highest(c1) == highest(c2)){
//  unknown[c1].splice(unknown[c1].begin(), unknown[c2]);
    DL_CONCAT(unknown[c1], unknown[c2]);
    return;
  }
  point*pit, *tmp;
  DL_FOREACH(unknown[c2], pit){
    p[pit->x][pit->y] = e[pit->x][pit->y]-h;
  }
  DL_FOREACH_SAFE(unknown[c2], pit, tmp){// clear
    DL_DELETE(unknown[c2], pit);         // clear
    free(pit);                           // clear
  }                                      // clear
}
int find_(int x, int y){
...
int highest(int cc){
  point*pdt = unknown[cc];
  return e[pdt->x][pdt->y];
}
